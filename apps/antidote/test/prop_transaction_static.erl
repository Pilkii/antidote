...
%% Model Callbacks
-export([command/1, initial_state/0, next_state/3, adjustCounter/2,
...
postcondition(#state{node = Node, object = Object, count = Count, clock = Clock}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, Op, Int}]]]}, {ok, _}) ->
    {ok, [Val], _Clock} = rpc:call(Node, antidote, read_objects, [Clock, [], [Object]]),
    adjustCounter(Count, {Op, Int}) =:= Val;
...
next_state(State=#state{count = Count}, {ok, NewClock}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, Op, Int}]]]}) ->
    NewState = State#state{count = adjustCounter(Count, {Op, Int}), clock = NewClock},
    NewState;
...
adjustCounter(Count, []) ->
    Count;
adjustCounter(Count, {increment, Int}) ->
    Count + Int;
adjustCounter(Count, {decrement, Int}) ->
    Count - Int;
adjustCounter(Count, [Op | Ops]) ->
    adjustCounter(adjustCounter(Count, Ops), Op).
