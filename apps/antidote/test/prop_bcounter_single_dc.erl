-module(prop_bcounter_single_dc).
...
-define(CRDT, antidote_crdt_counter_b).

%%%%%%%%%%%%%%%%%%
%%% PROPERTIES %%%
%%%%%%%%%%%%%%%%%%
prop_bcounter_single_dc_test() ->
...
initial_state() ->
    Bucket = test_bucket_bcounter_single_dc, 
    Key = antidote_prop_bcounter_single_dc,
    Node = persistent_term:get(node),
    Type = ?CRDT,
    Object = {Key, Type, Bucket},
    {ok, [Val], NewClock} = rpc:call(Node, antidote, read_objects, [ignore, [], [Object]]),
    Update = {Object, reset, {}},
    X = rpc:call(Node, antidote, update_objects, [NewClock, [], [Update]]),
    io:format("Init: ~p , now: ~p", [Val , X]),
...
postcondition(_, {call, _Mod, _Fun, [_, _, _, _]}, _) ->
    true.
