...
% next_state(State = #state{set = Set}, {ok, Clock}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, add, Elem}]]]}) ->
%     ResSet = lists:usort(lists:append([Elem], Set)),
%     NewState = State#state{set = ResSet, clock = Clock},
%     NewState;
% next_state(State = #state{set = Set}, {ok, Clock}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, remove, Elem}]]]}) ->
%     ResSet = lists:delete(Elem, Set),
%     NewState = State#state{set = ResSet, clock = Clock},
%     NewState;
% next_state(State = #state{set = Set}, {ok, Clock}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, add_all, ElemList}]]]}) ->
%     ResSet = lists:usort(lists:append(ElemList, Set)),
%     NewState = State#state{set = ResSet, clock = Clock},
%     NewState;
% next_state(State = #state{set = Set}, {ok, Clock}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, remove_all, ElemList}]]]}) ->
%     ResSet = lists:subtract(Set, ElemList),
%     NewState = State#state{set = ResSet, clock = Clock},
%     NewState;
