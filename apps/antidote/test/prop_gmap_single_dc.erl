...
-define(CRDT, antidote_crdt_map_go).

%%%%%%%%%%%%%%%%%%
%%% PROPERTIES %%%
%%%%%%%%%%%%%%%%%%
prop_gmap_single_dc_test() ->
...
initial_state() ->
    Bucket = test_bucket_gmap_single_dc, 
    Key = antidote_prop_gmap_single_dc,
    Node = persistent_term:get(node),
    Type = ?CRDT,
    Object = {Key, Type, Bucket},
    {ok, [Val], NewClock} = rpc:call(Node, antidote, read_objects, [ignore, [], [Object]]),
    io:format("Val:: ~p", [Val]),
    Update = {Object, update, {{<<"f">>, antidote_crdt_counter_pn}, {increment , 7}}},
    io:format("Update:: ~p", [Update]),
    Res = rpc:call(Node, antidote, update_objects, [NewClock, [], Update]),
    io:format("Ress ~p", [Res]),
...
postcondition(_, _, Res) ->
    io:format(" postcondition: ~p ", [Res]),
    true.
