-module(prop_lwwregister_single_dc).
...
-define(CRDT, antidote_crdt_register_lww).

%%%%%%%%%%%%%%%%%%
%%% PROPERTIES %%%
%%%%%%%%%%%%%%%%%%
prop_lwwregister_single_dc_test() ->
...
initial_state() ->
    Bucket = test_bucket_lwwregister_single_dc, 
    Key = antidote_prop_lwwregister_single_dc,
...
postcondition(#state{reg = Reg, node = Node, object = Object}, {call, _Mod, _Fun, [_, _, update_objects, [_, _, [{_, Op, OpArg}]]]}, {ok, Clock}) ->
    ...
    if 
        not X -> io:format("Post False NewMap: ~p, ReadMap: ~p ", [NewReg, ReadReg])
                , X;
        true -> X
    end;
...
postcondition(#state{reg = Reg}, {call, _Mod, _Fun, [_, _, read_objects, _]}, Res) ->
    ...
    if 
        not X -> io:format("Post False NewMap: ~p, ReadMap: ~p ", [Reg, ReadReg])
                , X;
        true -> X
    end.
